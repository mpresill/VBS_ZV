...
//Input PUId File name
string filename = "/afs//cern.ch/user/a/ahakimi/ZV_analysis/mkShapePyRDF/patches/PUID_81XTraining_EffSFandUncties.root";
string year = "2018";
string wp_ = "L";


void init(string filename, string year, string wp_ ) {
...
RVec<float> jets_cat(int nCleanJetNotFat,int  nCleanFatJet,RVec<float> & CleanJet_pt, RVec<int> & CleanJetNotFat_jetId, RVec<float> & CleanJet_phi, RVec<float> & CleanJet_eta, RVec<float> & Jet_mass, RVec<int> & CleanJet_jetId, RVec<float> & CleanFatJet_mass, int nLepton, RVec<float> & Lepton_eta, RVec<float> & CleanFatJet_eta, RVec<float> & Jet_qgl, RVec<int> & CleanFatJet_jetid, RVec<float> & Jet_btagDeepB, int year){
...
float sum_eta_lep=0;
unsigned int njet=nCleanJetNotFat;
unsigned int nFJ=nCleanFatJet;
...
else if (year == 2017) btagcut = 0.1522;
else if (year == 2016) btagcut = 0.2217;
else cout << "Please enter valid year" <<endl;
...
if (jet0.Pt()>pt_cut){
  ...
  if(Jet_btagDeepB.at(CleanJet_jetId.at(CleanJetNotFat_jetId.at(ijet)))>=btagcut && std::abs(jet0.Eta() )<2.5){
            _nbtag ++;
            }
  }
}
...
if (njet>=2){
	sum_eta_lep=std::abs(Lepton_eta[0]+Lepton_eta[1]);
	...
	for (unsigned int ijet=0 ; ijet<njet; ijet++){
			...
			Mjj_tmp = (jet0 + jet1).M();
    			...
    			if( Mjj_tmp >= Mjj_max ){
        			...
        			mean_eta_vbs=CleanJet_eta.at(CleanJetNotFat_jetId.at(ijet))+CleanJet_eta.at(CleanJetNotFat_jetId.at(jjet))*0.5;
        // Index in the collection of CleanJetNotFat
        			VBS_jets[0]= ijet;
        			VBS_jets[1]= jjet;
    			}
...
//cout << "boosted" <<endl;
    category = 0;
    Vjet_mass_tmp = CleanFatJet_mass[0];
...
if (VBS_jets[0] != 999) vbs_jet_0 = CleanJetNotFat_jetId.at(vectors_id.at(VBS_jets[0]));
if (VBS_jets[1] != 999) vbs_jet_1 = CleanJetNotFat_jetId.at(vectors_id.at(VBS_jets[1]));

if (V_jets[0] != 999) v_jet_0 = CleanJetNotFat_jetId.at(vectors_id.at(V_jets[0]));

if (V_jets[1] != 999) v_jet_1= CleanJetNotFat_jetId.at(vectors_id.at(V_jets[1]));

//cout << vbs_jet_0 << " jets " << vbs_jet_1 <<endl;
...
float Zlep_2=999;
float Zfatjet = 999;
float Njet = (float)njet;
//float Czv = 999;
if (vbs_jet_0 != 999 && vbs_jet_1 !=999){
	Zlep_1=(Lepton_eta[0]-0.5*(CleanJet_eta.at(vbs_jet_0)+CleanJet_eta.at(vbs_jet_1)))/detajj_mjj_max;
	Zlep_2=(Lepton_eta[1]-0.5*(CleanJet_eta.at(vbs_jet_0)+CleanJet_eta.at(vbs_jet_1)))/detajj_mjj_max;
    if (category==0){
        Zfatjet = (CleanFatJet_eta[0] - 0.5 * (CleanJet_eta.at(vbs_jet_0) + CleanJet_eta.at(vbs_jet_1))) / detajj_mjj_max;
    }
}

RVec<float> cat{Mjj_max,detajj_mjj_max,dphijj_mjj_max,Vjet_mass_tmp, Zepp_ll, Zlep_1, Zlep_2, 999,999,999,999,999,999,999,999, category, Njet, Zfatjet, 999, 999,999,999,999,999};
cat[23] = _nbtag;
if (vbs_jet_0 != 999){
     cat[7]= CleanJet_pt.at(vbs_jet_0);
     cat[9]= CleanJet_eta.at(vbs_jet_0);
     cat[18] = Jet_qgl.at(CleanJet_jetId.at(vbs_jet_0));
}
if (vbs_jet_1 != 999){
     cat[8]= CleanJet_pt.at(vbs_jet_1);
     cat[10]= CleanJet_eta.at(vbs_jet_1);
     cat[19] = Jet_qgl.at(CleanJet_jetId.at(vbs_jet_1));
}
...
if (category==1){
    if (v_jet_0 != 999){
        ...
        cat[20] = Jet_qgl.at(CleanJet_jetId.at(v_jet_0));
    }
    if (v_jet_1 != 999){
    cat[12]=CleanJet_pt.at(v_jet_1);
    cat[14]=CleanJet_eta.at(v_jet_1);
    cat[21] = Jet_qgl.at(CleanJet_jetId.at(v_jet_1));
    }
}
if (category==0) {
    cat[22] = Jet_qgl.at(CleanFatJet_jetid[0]);
}
