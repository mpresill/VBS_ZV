...
ptllDYW_NLO = '( \
  0.87*(gen_ptll<10) + \
  (0.379119 + 0.099744*gen_ptll - 0.00487351*pow(gen_ptll,2) + 9.19509e-05*pow(gen_ptll,3) - 6.0212e-07*pow(gen_ptll,4) )*\
  (gen_ptll>=10 && gen_ptll<45)+\
  (9.12137e-01 + 1.11957e-04*gen_ptll - 3.15325e-06*pow(gen_ptll,2) - 4.29708e-09*pow(gen_ptll,3) + 3.35791e-11*pow(gen_ptll,4) )*\
  (gen_ptll>=45 && gen_ptll<200) + \
  1*(gen_ptll>200)\
  )'
...
for _, sd in DataRun:
  for pd in DataSets:
    ...
    files = nanoGetSampleFiles(directory_data, pd + '_' + sd)[0:20]
